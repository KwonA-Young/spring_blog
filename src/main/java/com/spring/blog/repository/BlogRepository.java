...
public interface BlogRepository {
    // 사전에 정의해야 하는 메서드들.
    // 테이블 생성.
    void createBlogTable();    // 이 메서드 호출 시 CREATE TABLE 구문 실행.
    ...
    // 새 데이터 저장 기능 save()
    // 저장시 writer, blog_title, blog_content 3개 파라미터를 요구한다.
    // 근데 위 3개 파라미터는 Blog 엔터티의 멤버변수이다.
    void save(Blog blog);

    // 데이터 삭제 기능 deleteById()
    // 삭제시 삭제에 필요한 primary key 에 해당하는 아이디값을 요구
    void deleteById(long blogId);

    // 데이터 수정 기능 update()
    // JPA 에서는 .save()를 동일하게 사용하지만, 현재 코드에서 메서드 오버로딩도 불가능하고
